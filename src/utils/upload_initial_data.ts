...
// const updateStorageImages = async () => {
//     try {
//         const images = await fs.readdir('./src/assets/images')

//         Promise.all(images.map(async (image) => {
//             const imagePath = `./src/assets/images/${image}`
//             const resizedImagePath = `./src/assets/resized-images/${image}`

//             sharp(imagePath).resize(250, 250).toFile(resizedImagePath)

//             const fileData = await fs.readFile(resizedImagePath);

//             const { error } = await supabase.storage
//                 .from('images')
//                 .upload(image, fileData, { upsert: true }); 
//                 // Upsert para que no de error y se sobreescriban las imágenes, dado que ya estaban antes en el bucket.
//                 // Quitar el upsert si es la primera vez que se suben las imágenes.
